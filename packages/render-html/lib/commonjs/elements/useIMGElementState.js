...
function getImageSizeAsync({
  ...
  return new Promise((onsuccess, onerror) => {
    ...
    const cachedImage = _CachedImage.ImageCache.get().getCachedImagePath(uri);
    ...
    if (cachedImagePath) {
      ...
      _reactNativeFetchBlob.default.fs.exists(cachedImagePath).then(exists => {
        if (exists) {
          _reactNative.Image.getSize(cachedImagePath, onImageDimensionsSuccess, onerror);
        } else {
          if (headers) {
